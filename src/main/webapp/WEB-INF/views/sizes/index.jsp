...
<div class="container bg-white" style="padding: 10px; border-radius: 10px;">
    ...
    <table class="table table-striped ">
        ...
        <tbody>
        <c:forEach var="s" items="${sizes.content}" varStatus="i">
            <tr>
                <th scope="row">${i.index + 1}</th>
                <td>${s.id}</td>
                <td>${s.code}</td>
                <td>${s.name}</td>
                <td class="${s.status ? "text-danger" : "text-success" }">${s.status ? "Không dùng" : "Đang kinh doanh"}</td>
                <td>
                    <a href="/shop-app/sizes/view-update?id=${s.id}&page=${currentPage}&size=${currentSize}"
                       class="btn btn-warning">Edit</a>
                    <a href="/shop-app/sizes/detail?id=${s.id}&page=${currentPage}&size=${currentSize}"
                       class="btn btn-info">Info</a>
                    <a href="/shop-app/sizes/update-status?id=${s.id}"
